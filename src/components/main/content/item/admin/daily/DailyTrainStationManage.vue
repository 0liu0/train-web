  ...
  <a-modal v-model:visible="visible" title="每日车站" @ok="handleOk"
    ...
    <a-form :model="dailyTrainStation" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="进站时间">
        <a-time-picker v-model:value="dailyTrainStation.inTime" valueFormat="HH:mm" placeholder="请选择时间" />
      </a-form-item>
      <a-form-item label="出站时间">
        <a-time-picker v-model:value="dailyTrainStation.outTime" valueFormat="HH:mm" placeholder="请选择时间" />
      </a-form-item>
      <a-form-item label="停站时长">
        <a-time-picker v-model:value="dailyTrainStation.stopTime" valueFormat="HH:mm" placeholder="请选择时间" disabled/>
      </a-form-item>
...
import { Tool } from '../../../../../../utils/tool';
    ...
    watch(() => dailyTrainStation.value.inTime, ()=>{
      ...
      dailyTrainStation.value.stopTime = dayjs('00:00:00', 'HH:mm').second(diff).format('HH:mm');
    }, {immediate: true});
    ...
    const onDelete = (record) => {
      myAxios.delete("/business/admin/daily-train-station/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      myAxios.post("/business/admin/daily-train-station/save", dailyTrainStation.value).then((response) => {
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      myAxios.get("/business/admin/daily-train-station/query-list", {
        params: {
          page: param.page,
          size: param.size,
