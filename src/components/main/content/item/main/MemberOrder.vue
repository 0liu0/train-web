...
export default defineComponent({
  ...
  setup() {
    ...
    const dailyTrainTicket = Tool.getFromSessionStorage('SESSION_ORDER') || {};
    ...
    const SEAT_COL_ARRAY = computed(() => {
      console.log("lzcddd:",store.state.SEAT_COL_ARRAY)
      return store.state.SEAT_COL_ARRAY.filter(item => item.type === chooseSeatType.value);
    });
    // 选择的座位
    // {
    ...
    const handleQueryPassenger = () => {
      myAxios.get("/member/passenger/query-mine").then((response) => {
    ...
    const handleOk = () => {
      ...
      myAxios.post("/business/confirm-order/do", {
